            ...
            <DialogContent className="max-w-[95vw] w-full h-[95vh] p-0 gap-0 overflow-hidden bg-gray-100 flex flex-col sm:flex-row">
                ...
                <div className="w-[400px] bg-white border-l flex flex-col  z-10">
                    ...
                    <div className="p-4 space-y-3 flex-1 overflow-y-auto">
                        ...
                        {/* Injectable Orders Status Panel */}
                        {forwardingStatus && forwardingStatus.nursing?.sent && (
                            <div className="mt-4 border rounded-lg overflow-hidden bg-gradient-to-b from-blue-50 to-white border-blue-200">
                                <div className="bg-blue-600 text-white px-3 py-2 flex items-center gap-2">
                                    <Syringe className="w-4 h-4" />
                                    <span className="text-xs font-semibold uppercase tracking-wider">Ordens de Injetáveis</span>
                                    <span className="ml-auto bg-white/20 rounded-full px-2 py-0.5 text-[10px] font-bold">
                                        {forwardingStatus.nursing.orderCount || 1}
                                    </span>
                                </div>
                                <div className="p-3 space-y-2">
                                    ...
                                    {forwardingStatus.nursing.routes && forwardingStatus.nursing.routes.length > 0 && (
                                        <div className="flex flex-wrap gap-1.5">
                                            {forwardingStatus.nursing.routes.map((route, idx) => (
                                                <div
                                                    key={idx}
                                                    className="flex items-center gap-1.5 bg-blue-100 text-blue-800 px-2.5 py-1 rounded-md text-xs font-medium border border-blue-200"
                                                >
                                                    <Syringe className="w-3 h-3" />
                                                    Via {route}
                                                    <CheckCircle2 className="w-3 h-3 text-green-600" />
                                    ...
                                    <div className="flex items-center gap-2 text-[11px] text-gray-600">
                                        ...
                                        <span>
                                            {forwardingStatus.nursing.orderCount === 1
                                                ? 'Ordem gerada e enviada para aplicações'
                                                : `${forwardingStatus.nursing.orderCount} ordens geradas e enviadas`
                                            }
                                        </span>
                                    </div>

                                    {forwardingStatus.nursing.sentAt && (
                                        <div className="flex items-center gap-2 text-[10px] text-gray-400">
                                            <Clock className="w-3 h-3" />
                                            <span>
                                                Gerado em {new Date(forwardingStatus.nursing.sentAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                                            </span>
                                        </div>
                                    )}
                                    ...
                                    <a
                                        ...
                                        className="flex items-center gap-1.5 text-[11px] text-blue-600 hover:text-blue-800 hover:underline mt-1"
                                    >
                                        <Send className="w-3 h-3" />
                                        Ver na lista de aplicações →
                                    </a>
                                </div>
                            </div>
                        )}

                        {/* Pharmacy forwarding status */}
                        {forwardingStatus && forwardingStatus.pharmacy?.sent && (
                            <div className="mt-2 border rounded-lg overflow-hidden bg-gradient-to-b from-emerald-50 to-white border-emerald-200">
                                <div className="bg-emerald-600 text-white px-3 py-2 flex items-center gap-2">
                                    <Building2 className="w-4 h-4" />
                                    <span className="text-xs font-semibold uppercase tracking-wider">Farmácia</span>
                                    {forwardingStatus.pharmacy.status === 'confirmed' || forwardingStatus.pharmacy.status === 'delivered' ? (
                                        <CheckCircle2 className="w-3.5 h-3.5 ml-auto" />
                                    ) : (
                                        <Clock className="w-3.5 h-3.5 ml-auto text-amber-200" />
                                    )}
                                </div>
                                <div className="p-3 text-[11px] text-gray-600 space-y-1">
                                    {forwardingStatus.pharmacy.partnerName && (
                                        <p><span className="font-medium">Parceiro:</span> {forwardingStatus.pharmacy.partnerName}</p>
                                    )}
                                    {forwardingStatus.pharmacy.responsibleName && (
                                        <p><span className="font-medium">Responsável:</span> {forwardingStatus.pharmacy.responsibleName}</p>
                                    )}
                                    <p className="text-gray-400 capitalize">Status: {forwardingStatus.pharmacy.status || 'enviado'}</p>
                                </div>
                            </div>
                        )}
                        ...
                        {/* No orders generated */}
                        {(!forwardingStatus || (!forwardingStatus.nursing?.sent && !forwardingStatus.pharmacy?.sent)) && (
                            <div className="mt-4 border rounded-lg p-3 bg-gray-50 border-gray-200 text-center">
                                <Syringe className="w-5 h-5 text-gray-300 mx-auto mb-1.5" />
                                <p className="text-[11px] text-gray-400">Nenhuma ordem de injetável gerada nesta receita</p>
                                <p className="text-[10px] text-gray-300 mt-0.5">Selecione uma fórmula injetável para gerar automaticamente</p>
                            </div>
                        )}

                        <div className="mt-4 border rounded-lg p-3 bg-primary/10 border-primary/30">
