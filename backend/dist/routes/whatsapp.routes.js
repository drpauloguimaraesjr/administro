...
router.post('/message', async (req, res) => {
    try {
        ...
        if (!transactionData) {
            ...
            await sendWhatsAppMessage(from, '❓ Não consegui entender. Envie no formato:\n\n💰 R$ 50,00\n📝 Descrição\n🏷️ Categoria\n🏠 Casa ou Clínica\n\nOu envie uma imagem do comprovante!');
            return res.json({
                success: false,
 ...
 */
async function extractTransactionFromText(text, from, fromName) {
    // Normaliza texto
    const normalizedText = text.toLowerCase().trim();
    // Extrai valor usando regex
    const amountMatch = normalizedText.match(/(?:r\$\s*)?(\d+(?:[.,]\d{2})?)|(\d+)\s*(?:reais?|rs?)/i);
    if (!amountMatch) {
    ...
    const isIncome = normalizedText.includes('recebi') ||
    ...
    // Detecta contexto
    let contextId = 'HOME';
    if (normalizedText.includes('clínica') ||
