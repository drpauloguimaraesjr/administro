 ...
 */
import makeWASocket, { DisconnectReason, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, initAuthCreds, BufferJSON, } from '@whiskeysockets/baileys';
...
async function updateFirestoreStatus(status, qr) {
    try {
...
async function handleIncomingMessage(message) {
    try {
        ...
        if (!from || from.includes('@g.us') || from.includes('status@broadcast'))
            return;
...
async function processTextMessage(from, text, pushName, messageId) {
...
async function processMediaMessage(from, type, pushName, messageId, mediaContent) {
 ...
 * Envia documento/PDF
 */
export async function sendDocument(to, url, filename) {
    if (!socket || connectionState !== 'open') {
        console.error('WhatsApp não conectado');
        return false;
    }
