...
import { WhatsAppQueue } from '../shared/types/index.js';

const COLLECTION = 'whatsapp_queues';

export const WhatsAppQueuesService = {
    async getAll(): Promise<WhatsAppQueue[]> {
        const snapshot = await db.collection(COLLECTION).get();
        return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as WhatsAppQueue));
    },

    async getById(id: string): Promise<WhatsAppQueue | null> {
        const doc = await db.collection(COLLECTION).doc(id).get();
        if (!doc.exists) return null;
        return { id: doc.id, ...doc.data() } as WhatsAppQueue;
    },

    async create(data: Omit<WhatsAppQueue, 'id' | 'createdAt'>): Promise<WhatsAppQueue> {
        const docRef = db.collection(COLLECTION).doc();
        const now = new Date().toISOString();

        const queueData: WhatsAppQueue = {
            id: docRef.id,
            ...data,
            createdAt: now,
            isActive: true,
    ...
    async update(id: string, data: Partial<WhatsAppQueue>): Promise<WhatsAppQueue | null> {
        const docRef = db.collection(COLLECTION).doc(id);
        await docRef.update(data);
        const updated = await docRef.get();
        return { id: updated.id, ...updated.data() } as WhatsAppQueue;
    },
    ...
    async seedDefaults(): Promise<void> {
        ...
        const DEFAULT_QUEUES: Partial<WhatsAppQueue>[] = [
            {
                name: "🔥 Aquecimento",
