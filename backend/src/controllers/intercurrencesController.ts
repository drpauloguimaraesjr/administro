...
import { z } from 'zod';
import { Intercurrence } from '../shared/types/index.js';
...
export const getIntercurrences = async (req: Request, res: Response) => {
    try {
...
export const createNewIntercurrence = async (req: Request, res: Response) => {
    ...
    try {
        ...
        } as unknown as Omit<Intercurrence, 'id'>;
...
export const updateIntercurrenceStatus = async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    try {
        const updateData: Partial<Intercurrence> = { updatedAt: new Date().toISOString() };
        if (status) updateData.status = status;
        if (aiAnalysis) updateData.aiAnalysis = aiAnalysis;

        const updated = await updateIntercurrence(id, updateData);
        if (!updated) return res.status(404).json({ error: 'Intercurrence not found' });
        res.json(updated);
    } catch (error) {
