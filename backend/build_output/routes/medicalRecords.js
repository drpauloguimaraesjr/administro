...
router.get('/:patientId/anamnesis', async (req, res) => {
    try {
...
router.post('/:patientId/anamnesis', async (req, res) => {
    try {
        const db = getFirestore();
...
router.get('/:patientId/evolutions', async (req, res) => {
    try {
        const db = getFirestore();
...
router.post('/:patientId/evolutions', async (req, res) => {
    try {
        const db = getFirestore();
        const now = new Date().toISOString();
        const evolution = {
...
router.put('/:patientId/evolutions/:id', async (req, res) => {
    try {
        const db = getFirestore();
...
router.delete('/:patientId/evolutions/:id', async (req, res) => {
    try {
        const db = getFirestore();
        await db.collection('medical_records')
            .doc(req.params.patientId)
...
router.post('/prescriptions/generate-title', async (req, res) => {
    try {
...
function extractKeywords(text) {
    ...
    if (/uso injet[aá]vel/i.test(text))
        return 'Medicação Injetável';
    if (/uso oral/i.test(text))
        return 'Medicação Oral';
    if (/uso t[oó]pico/i.test(text))
        return 'Medicação Tópica';
...
router.put('/templates/:id/favorite', async (req, res) => {
    try {
        const db = getFirestore();
        const docRef = db.collection('prescription_templates').doc(req.params.id);
        const doc = await docRef.get();
