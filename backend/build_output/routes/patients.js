...
router.get('/', async (req, res) => {
    try {
        ...
        // Filter by search
        if (search) {
            patients = patients.filter(p => p.name?.toLowerCase().includes(search) ||
...
router.get('/inactive', async (req, res) => {
    try {
        const db = getFirestore();
        const days = parseInt(req.query.days) || 90; // Default 90 dias
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const stats = {
            ...
            new: patients.filter((p) => p.tags?.includes('Novo')).length,
            recurring: patients.filter((p) => p.tags?.includes('Recorrente')).length,
            birthdaysThisMonth: patients.filter((p) => {
                if (!p.birthDate)
                    return false;
                return new Date(p.birthDate).getMonth() + 1 === currentMonth;
            }).length,
            newLast30Days: patients.filter((p) => {
                if (!p.createdAt)
                    return false;
                return new Date(p.createdAt) > thirtyDaysAgo;
            }).length,
            inactive90Days: patients.filter((p) => {
                const lastVisit = new Date(p.lastVisit || p.lastAppointment || p.createdAt || '2000-01-01');
                return lastVisit < ninetyDaysAgo;
            }).length,
            // Grade distribution
            gradeAAA: patients.filter((p) => p.grade === 'AAA' || (p.score || 0) >= 80).length,
            gradeAA: patients.filter((p) => p.grade === 'AA' || ((p.score || 0) >= 60 && (p.score || 0) < 80)).length,
            gradeA: patients.filter((p) => p.grade === 'A' || ((p.score || 0) >= 40 && (p.score || 0) < 60)).length,
            gradeB: patients.filter((p) => p.grade === 'B' || ((p.score || 0) >= 20 && (p.score || 0) < 40)).length,
            gradeC: patients.filter((p) => p.grade === 'C' || (p.score || 0) < 20).length,
        };
        res.json(stats);
    }
    catch (error) {
