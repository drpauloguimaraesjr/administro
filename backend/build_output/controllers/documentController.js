...
import { compileToPdf, deletePdf } from '../services/typstService.js';
...
export const createDocument = async (req, res) => {
    try {
        const { type, data, addToQueue = true } = req.body;
        if (!type || !data) {
 ...
 */
export const getSignatureQueue = async (req, res) => {
    try {
 ...
 */
export const signDocumentsBatch = async (req, res) => {
    try {
        const { documentIds, birdIdCode } = req.body;
...
export const getDocuments = async (req, res) => {
    try {
        ...
        if (patientName) {
            ...
            documents = documents.filter(d => d.patientName?.toLowerCase().includes(searchTerm));
        }
        res.json({
