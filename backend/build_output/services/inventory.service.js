...
export async function getBatches(itemId) {
    let query = db.collection('inventory_batches').orderBy('expirationDate', 'asc');
    if (itemId) {
...
// Movimentações
// =====================
export async function getMovements(itemId, limit = 100) {
...
export async function getAlerts(status) {
    let query = db.collection('inventory_alerts')
        .orderBy('createdAt', 'desc');
    if (status) {
...
// Análise de Consumo
// =====================
export async function analyzeConsumption(itemId, periodDays = 30) {
...
export async function getConsumptionSummary(periodDays = 30) {
    const items = await getAllItems();
    const analyses = [];
    for (const item of items) {
